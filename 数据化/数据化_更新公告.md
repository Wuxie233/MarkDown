...
<hr>
<h1 id="v260323"><span class="prefix"></span><span class="content">v260323</span><span class="suffix"></span></h1>
<p>// Bug修复 + 个人信息优化</p>
<ul>
<li>修复 满血时仍持续触发回血<br>
<code>2-1周期回血增加满血检测 当前血量>=最大血量时不再写入回血接口 不会出现满血还一直弹回血提示和爱心粒子的问题</code></li>
<li>修复 个人信息查询执行不完整<br>
<code>8-1个人信息原有约45条命令方块 超出网易链式CB约20条的执行上限 导致后半部分指令无法执行 现已将所有tellraw通过换行符合并 总命令数降至14条</code></li>
<li>优化 个人信息武器/防具/护符属性显示<br>
<code>武器/防具/护符属性改为分组合并显示 每类装备的全部属性在一条tellraw中输出 带分隔线 信息更紧凑整齐</code></li>
</ul>
<blockquote>
<p><strong>升级说明</strong>：本次更新无新增计分板 直接替换<code>2-1周期回血</code>和<code>8-1个人信息</code>两个模块的BDX即可 其他模块无需修改</p>
</blockquote>
<h1 id="v260322"><span class="prefix"></span><span class="content">v260322</span><span class="suffix"></span></h1>
<p>// 战斗系统架构重构 + 性能优化</p>
<ul>
<li>重构 统一伤害引擎<br>
<code>新增3-5伤害引擎 将近战和远程的伤害计算统一到一个引擎中 受击算法和弓箭受击不再各自计算 而是通过计分板接口调用伤害引擎 修复了弓箭伤害不吃护盾和吸血的问题 修复了直接修改血量绕过闪避/防御/准确的问题</code></li>
<li>重构 受击算法(3-3)和弓箭受击(4-3)<br>
<code>两个文件大幅精简 只负责识别攻击者身份和复制攻击者属性 伤害计算全部交给伤害引擎处理</code></li>
<li>修复 天赋升级后属性面板不自动刷新<br>
<code>天赋升级/天赋洗点/玩家升级 三处均缺少触发属性更新的tag 导致加点后面板属性不会立刻重算 现已修复</code></li>
<li>修复 伤害引擎暴击UI判定对象错误</li>
<li>修复 伤害浮动拓展弓箭浮动值写入错误的计分板</li>
<li>优化 全实体扫描移到低频链<br>
<code>新增2-9怪物维护(低频链) 将原本每tick执行的怪物effect和初始化操作改为每秒执行一次 减少服务器空闲时的算力消耗 适配低配租赁服</code></li>
<li>新增 5个伤害引擎接口计分板<br>
<code>结算伤害/结算_准确/结算_穿透/结算_暴击/结算_暴倍 使用一键适配自动创建</code></li>
<li>新增 伤害引擎简化调用接口<br>
<code>自制技能只需设置 结算伤害 + tag伤害结算 + tag_结算来源 引擎自动从攻击者读取准确/穿透/暴击/暴倍面板属性 方便二次开发用户快速对接伤害系统</code></li>
<li>新增 计分板GC机制(定时链)<br>
<code>新增2-10计分板GC 使用#[]定时链每30分钟自动清理死亡实体残留的计分板数据 清理22个A类纯实体/临时计分板 备份恢复2个C类共用计分板(当前/最大血量) 内含三重安全机制防止离线玩家上线后死循环</code></li>
<li>新增 统一内存释放安全网<br>
<code>2-2死亡检测末尾增加兜底清理 确保所有标记tag=内存释放的实体必定在1tick内被kill</code></li>
<li>修复 新玩家初始化后属性面板未立即计算<br>
<code>1-3分配属性末尾补充tag=属性更新 新玩家进服后立即触发面板属性重算</code></li>
</ul>
<blockquote>
<p><strong>升级说明</strong>：从旧版本升级需重新运行<code>一键适配</code>以创建新计分板（GC备份等）。新增的<code>3-5伤害引擎</code>、<code>2-9怪物维护</code>、<code>2-10计分板GC</code>需在一键导入中分配命令方块坐标并编译BDX。其中<code>2-10</code>为定时链(#[])结构 编译后需在压力板上方放置盔甲架。已有的怪物、装备、拓展模块无需修改即可兼容。</p>
</blockquote>
<ul>
<li>新增计分板</li>
</ul>
<blockquote>
<p>scoreboard objectives add 结算伤害 dummy<br>
scoreboard objectives add 结算_准确 dummy<br>
scoreboard objectives add 结算_穿透 dummy<br>
scoreboard objectives add 结算_暴击 dummy<br>
scoreboard objectives add 结算_暴倍 dummy<br>
scoreboard objectives add GC备份 dummy</p>
</blockquote>
<h1 id="v240718"><span class="prefix"></span><span class="content">v240718</span><span class="suffix"></span></h1>
<ul>
<li>修复 怪物参数 掉落方式=1 掉落木桶全部道具（已修复为掉落木桶里随机一个道具）</li>
</ul>
